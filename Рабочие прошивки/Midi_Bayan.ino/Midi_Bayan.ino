...
//  D0      , D1      , D2      , D3      , D4      , D5     , D6 ,       D7 
  ...
  {{Ab6,Ab6},{E6 ,E6 },{Db6,Db6},{A5 ,A6 },{Fd5,Fd6},{D5 ,D6 },{B4 ,B5 },{Ab4,Ab5}},  // Для L0 
                                                     //Ближний правый ряд, снизу вверх
  {{E4 ,E5 },{Db4,Db5},{A3 ,A5 },{F4 ,F5 },{C4 ,C5 },{A6 ,A5 },{Fd6,Fd6},{D6 ,D6 }},  // Для L1 
  {{B5 ,B6 },{Ab5,Ab6},{E5 ,E6 },{Db5,Db6},{A4 ,A5 },{Fd4,Fd5},{D4 ,D5 },{B3 ,B4 }},  // Для L2 
  {{Eb4,Eb5},{NNN,NNN},{NNN,NNN},{NNN,NNN},{NNN,NNN},{NNN,NNN},{NNN,NNN},{NNN,NNN}},  // Для L3
  //Басы, сверху вниз  
  {{Ab2,Ab3},{A2 ,A3 },{B2 ,B3 },{Db3,Db2},{D3 ,D2 },{E3 ,E2 },{Fd3,Fd2},{NNN,NNN}},  // Для L4
  //Аккорды сверху вниз  
  {{Ab3,Ab4},{A3 ,A4 },{B3 ,B4 },{C4 ,C5 },{Db4,Db5},{D4 ,D5 },{Fd4,Fd5},{E4 ,E5 }},  // Для L5 
  {{F4 ,F5 },{Fd4,Fd5},{NNN,NNN},{NNN,NNN},{NNN,NNN},{NNN,NNN},{NNN,NNN},{NNN,NNN}},  // Для L6
  ...
  //if (datap[i] != data[i])
  {
    ...
    for (j=0; j<8; j++)  
    {
      ...
      if (( data[i] & (1<<j)) && (~datap[i] & (1<<j)))
      {
        ...
        if (md_active == 0)
        {
	      char note1 = notes[i][j][0];
	      char note2 = notes[i][j][1];
          if (md_chan_h == md_chan_l)
          {
			if(note1 != NNN)
				noteOn(0x80 | md_chan_h, note1, 0x00);
			if(note2 != NNN)
				noteOn(0x80 | md_chan_h, note2, 0x00);
          }
          else  
          {
			if(note1 != NNN)
			{
				noteOn(0x80 | md_chan_h, note1, 0x00);
				noteOn(0x80 | md_chan_l, note1, 0x00);
			}
            if(note2 != NNN)
			{
				noteOn(0x80 | md_chan_h, note2, 0x00);
				noteOn(0x80 | md_chan_l, note2, 0x00);
			}
           }
        }
      }
      ...
      if ((~data[i] & (1<<j)) && ( datap[i] & (1<<j)))
      {
        ...
        if (md_active == 0) 
        {
          ...
          if (md_chan_h == md_chan_l)
          {
            if (note1 != NNN && md_data.md_channel_volume[md_chan_h]>0) 
				noteOn(0x90 | md_chan_h, note1, 127);
			if (note2 != NNN && md_data.md_channel_volume[md_chan_h]>0) 
				noteOn(0x90 | md_chan_h, note2, 127);
          }
          else  
          {
			  if(note1 != NNN)
			  {
				if (md_data.md_channel_volume[md_chan_h]>0) 
					noteOn(0x90 | md_chan_h, note1, 127);
				if (md_data.md_channel_volume[md_chan_l]>0) 
					noteOn(0x90 | md_chan_l, note1, 127);  
			  }
			  if(note2 != NNN)
			  {
				if (md_data.md_channel_volume[md_chan_h]>0) 
					noteOn(0x90 | md_chan_h, note2, 127);
				if (md_data.md_channel_volume[md_chan_l]>0) 
					noteOn(0x90 | md_chan_l, note2, 127);  
			  }
          }
        }
      }
