...
// D0, D1,D2, D3,D4, D5,D6,D7
  ...
  {Eb4},                          // Для L3 
  {Ab2,A2,B2,Db3,D3,E3,Fd3},         // Для L4 Басы, сверху вниз
...
// Соответствие клавиш нотам. Каждая строка соответствует линиям:
// D0, D1,D2, D3,D4, D5,D6,D7
  {Ab5,E5,Db5,A4,Fd4,D4,B3,Ab3},  // Для L0 Дальний правый ряд, снизу вверх
  {E3, Db3,A3,F3,C3,              // Для L1 
                     A5,Fd5,D5},  // Для L1 Ближний правый ряд, снизу вверх
  {B4 ,Ab4,E4, Db4, A3,Fd3,D3, B3 }, // Для L2 
  {Eb3,NNN,NNN,NNN,NNN,NNN,NNN,NNN},                          // Для L3 
  {NNN,NNN,NNN,NNN,NNN,NNN,NNN,NNN},         // Для L4 Басы, сверху вниз
  {NNN,NNN,NNN,NNN,NNN,NNN,NNN,NNN},      // Для L5 Аккорды сверху вниз
  {NNN,NNN,NNN,NNN,NNN,NNN,NNN,NNN},                        // Для L6
// Например, кнопка, стоящая на пересечении D2 и L4 соответствует Eb4 
// NNN означает отсутствие ноты, кнопка не используется для извлечения звуков
  };
*/
...
// D0,  D1,  D2,  D3,  D4,  D5,  D6 , D7
  ...
  {0x03}, // Для L3
  {0x14,0x14,0x14,0x14,0x14,0x14,0x14}, // Для L4
      ...
      if (( data[i] & (1<<j)) && (~datap[i] & (1<<j)))
      {
        ...
        if ((notes[i][j] != NNN)  && (md_active == 0))
        {
          if (md_chan_h == md_chan_l)
          {
            ...
            /*if (notes2[i][j] != NNN)
                noteOn(0x80 | md_chan_h, notes2[i][j], 0x00);*/
          }
          else  
          {
            noteOn(0x80 | md_chan_h, notes[i][j], 0x00);
            ...
            {
              noteOn(0x80 | md_chan_h, notes2[i][j], 0x00);
              noteOn(0x80 | md_chan_l, notes2[i][j], 0x00);
            }
            */
           }
        }
