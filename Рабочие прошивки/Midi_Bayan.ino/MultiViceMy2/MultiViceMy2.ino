...
//  D0                  , D1                  , D2                  , D3                   , D4                  , D5                , D6                   , D7 
  //Дальний правый ряд, снизу вверх
  {{Ab6,Ab7,NNN,NNN,NNN},{E6 ,E7 ,NNN,NNN,NNN},{Db6,Db7,NNN,NNN,NNN},{A5 ,A6,NNN,NNN,NNN },{Fd5,Fd6,NNN,NNN,NNN},{D5 ,D6,NNN,NNN,NNN },{B4 ,B5 ,NNN,NNN,NNN},{Ab4,Ab5,NNN,NNN,NNN}},  // Для L0 
                                                                                                                 //Ближний правый ряд, снизу вверх
  {{E4 ,E5 ,NNN,NNN,NNN},{Db4,Db5,NNN,NNN,NNN},{A3 ,A5 ,NNN,NNN,NNN},{F4 ,F5,NNN,NNN,NNN },{C4 ,C5,NNN,NNN,NNN },{A6 ,A7,NNN,NNN,NNN },{Fd6,Fd7,NNN,NNN,NNN},{D6 ,D7 ,NNN,NNN,NNN}},  // Для L1 
  {{B5 ,B6 ,NNN,NNN,NNN},{Ab5,Ab6,NNN,NNN,NNN},{E5 ,E6 ,NNN,NNN,NNN},{Db5,Db6,NNN,NNN,NNN},{A4 ,A5,NNN,NNN,NNN },{Fd4,Fd5,NNN,NNN,NNN},{D4 ,D5 ,NNN,NNN,NNN},{B3 ,B4 ,NNN,NNN,NNN}},  // Для L2 
  {{Eb4,Eb5,NNN,NNN,NNN},{NNN,NNN,NNN,NNN,NNN},{NNN,NNN,NNN,NNN,NNN},{NNN,NNN,NNN,NNN,NNN},{NNN,NNN,NNN,NNN,NNN},{NNN,NNN,NNN,NNN,NNN},{NNN,NNN,NNN,NNN,NNN},{NNN,NNN,NNN,NNN,NNN}},  // Для L3
  //Басы, сверху вниз  
  {{Ab1,Ab2,Ab3,Ab4,Ab5},{A1 ,A2 ,A3 ,A4 ,A5 },{B1 ,B2 ,B3 ,B4 ,B5 },{Db1,Db2,Db3,Db4,Db5},{D1 ,D2 ,D3 ,D4 ,D5 },{E1 ,E2 ,E3 ,E4 ,E5 },{Fd1,Fd2,Fd3,Fd4,Fd5},{NNN,NNN,NNN,NNN,NNN}},  // Для L4
  //Аккорды сверху вниз  
  {{Ab3,Ab4,NNN,NNN,NNN},{A3 ,A4 ,NNN,NNN,NNN},{B3 ,B4 ,NNN,NNN,NNN},{C4 ,C5 ,NNN,NNN,NNN},{Db4,Db5,NNN,NNN,NNN},{D4 ,D5 ,NNN,NNN,NNN},{Fd4,Fd5,NNN,NNN,NNN},{E4 ,E5 ,NNN,NNN,NNN}},  // Для L5 
  {{F4 ,F5 ,NNN,NNN,NNN},{Fd4,Fd5,NNN,NNN,NNN},{NNN,NNN,NNN,NNN,NNN},{NNN,NNN,NNN,NNN,NNN},{NNN,NNN,NNN,NNN,NNN},{NNN,NNN,NNN,NNN,NNN},{NNN,NNN,NNN,NNN,NNN},{NNN,NNN,NNN,NNN,NNN}},  // Для L6
...
// D0, D1, D2, D3, D4, D5, D6 ,D7
  ...
  {_MV,___,___,___,___,___,___,___}, // Для L2
  ...
  //if (datap[i] != data[i])
  {
    ...
    for (j=0; j<8; j++)  
    {
      ...
      if (( data[i] & (1<<j)) && (~datap[i] & (1<<j)))
      {
        ...
        if (md_active == 0)
        {
			char note0 = notes[i][j][0];
			char note1 = notes[i][j][1];
			char note2 = notes[i][j][2];
			char note3 = notes[i][j][3];
			char note4 = notes[i][j][4];
			if (md_chan_h == md_chan_l)
			{
				if(note0 != NNN)
					noteOn(0x80 | md_chan_h, note0, 0x00);
				if(note1 != NNN)
					noteOn(0x80 | md_chan_h, note1, 0x00);
				}
			else  
			{
				//правая клавиатура
				if(md_chan_l == 0)
				{
					if(note0 != NNN) noteOn(0x80 | md_chan_l, note0, 0x00);
				}
				if(md_chan_h == 3)
				{
					if(note1 != NNN) noteOn(0x80 | md_chan_h, note1, 0x00);
				}
				//басы
				if(md_chan_l == 1)
				{
					if(note0 != NNN)noteOn(0x80 | md_chan_l, note0, 0x00);
					if(note1 != NNN)noteOn(0x80 | md_chan_l, note1, 0x00);
					if(note2 != NNN)noteOn(0x80 | md_chan_l, note2, 0x00);
					if(note3 != NNN)noteOn(0x80 | md_chan_l, note3, 0x00);
					if(note4 != NNN)noteOn(0x80 | md_chan_l, note4, 0x00);
				}
				if(md_chan_h == 4)
				{
					if(note0 != NNN)noteOn(0x80 | md_chan_h, note0, 0x00);
					if(note1 != NNN)noteOn(0x80 | md_chan_h, note1, 0x00);
					if(note2 != NNN)noteOn(0x80 | md_chan_h, note2, 0x00);
					if(note3 != NNN)noteOn(0x80 | md_chan_h, note3, 0x00);
					if(note4 != NNN)noteOn(0x80 | md_chan_h, note4, 0x00);
				}
				//аккорды
				if(md_chan_l == 2)
				{
      ...
      if ((~data[i] & (1<<j)) && ( datap[i] & (1<<j)))
      {
        ...
        if (md_active == 0) 
        {
          ...
          if (md_chan_h == md_chan_l)
          {
            if (note0 != NNN && md_data.md_channel_volume[md_chan_h]>0) 
				noteOn(0x90 | md_chan_h, note0, 127);
			if (note1 != NNN && md_data.md_channel_volume[md_chan_l]>0) 
				noteOn(0x90 | md_chan_h, note1, 127);
          }
          else  
          {			  		
				//правая клавиатура
				if(md_chan_l == 0 && md_data.md_channel_volume[md_chan_l] > 0)
				{
					if(note0 != NNN) 
						noteOn(0x80 | md_chan_l, note0, 127);
				}
				if(md_chan_h == 3 && md_data.md_channel_volume[md_chan_h] > 0)
				{
					if(note1 != NNN) 
						noteOn(0x80 | md_chan_h, note1, 127);
				}
				//басы
				if(md_chan_l == 1 && md_data.md_channel_volume[md_chan_l] > 0)
				{
					if(note0 != NNN)noteOn(0x80 | md_chan_l, note0, 127);
					if(note1 != NNN)noteOn(0x80 | md_chan_l, note1, 127);
					if(note2 != NNN)noteOn(0x80 | md_chan_l, note2, 127);
					if(note3 != NNN)noteOn(0x80 | md_chan_l, note3, 127);
					if(note4 != NNN)noteOn(0x80 | md_chan_l, note4, 127);
				}
				if(md_chan_h == 4 && md_data.md_channel_volume[md_chan_h] > 0)
				{
					if(note0 != NNN)noteOn(0x80 | md_chan_h, note0, 127);
					if(note1 != NNN)noteOn(0x80 | md_chan_h, note1, 127);
					if(note2 != NNN)noteOn(0x80 | md_chan_h, note2, 127);
					if(note3 != NNN)noteOn(0x80 | md_chan_h, note3, 127);
					if(note4 != NNN)noteOn(0x80 | md_chan_h, note4, 127);
				}
				//аккорды
				if(md_chan_l == 2 && md_data.md_channel_volume[md_chan_l] > 0)
				{
					if(note0 != NNN)noteOn(0x80 | md_chan_l, note0, 127);
					if(note1 != NNN)noteOn(0x80 | md_chan_l, note1, 127);
					if(note2 != NNN)noteOn(0x80 | md_chan_l, note2, 127);
				...
				if(md_chan_h == 5 && md_data.md_channel_volume[md_chan_h] > 0)
				{
					if(note0 != NNN)noteOn(0x80 | md_chan_h, note0, 127);
					if(note1 != NNN)noteOn(0x80 | md_chan_h, note1, 127);
					if(note2 != NNN)noteOn(0x80 | md_chan_h, note2, 127);
